...
lines = ['# Auto-generated', '', f'profile = \"{active}\"', '']
for key, p in providers.items():
    ...
    lines.append(f'model = \"{p.get(\"model\",\"gpt-4o\")}\"')
    lines.append(f'model_provider = \"{key}\"')
    lines.append('')
...
# 从 profiles.json 生成 Codex 原生 ~/.codex/config.toml
# Codex 只读这个文件，不读 profiles.json
generate_codex_toml() {
  [ ! -f "$CODEX_API_CONFIG_FILE" ] && return 0
...
print(f'Generated config.toml: active={active}, providers={list(filtered.keys())}')
...
generate_codex_toml_fallback() {
  ...
  # Build TOML line by line
  {
    ...
    while IFS= read -r line; do
      ...
      echo "$line" | grep -q '"env_key"' && current_envkey=$(echo "$line" | sed 's/.*"env_key"[[:space:]]*:[[:space:]]*"\([^"]*\)"/\1/')
  ...
  # Fix active profile if it was a reserved ID
  if echo "\"$active\"" | grep -qE "$reserved_regex"; then
    active="${first_custom:-}"
  fi
  if [ -n "$active" ]; then
    sed -i "1,/^profile = /s/^profile = .*/profile = \"$active\"/" "$CODEX_CONFIG_TOML"
  fi

  _logv "config.toml generated (fallback, no python3)"
}
