...
print(f'Generated config.toml: active={active}, providers={list(providers.keys())}')
" "$CODEX_API_CONFIG_FILE" "$CODEX_CONFIG_TOML" "$CODEX_DEFAULT_ENV_KEY" || { _logv "generate_codex_toml: Python 失败，尝试 fallback"; }
  else
    _warn "python3 not found, cannot generate config.toml (install python3 for full support)"
  fi
...
api_fetch_models() {
  ...
  _log "正在从 ${base_url}/models 拉取模型列表..."
