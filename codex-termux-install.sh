...
# --- 根据 Codex 版本决定是否传 --profile ---
# v0.142.0+ 需要 --profile + {profile}.config.toml
# v0.141.x 及更早使用 config.toml 中的 profile = "..." 字段
CODEX_RAW_VERSION="\$(\"\$CODEX_BIN\" --version 2>/dev/null | head -1 | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1)"
_need_profile=0
case "\$CODEX_RAW_VERSION" in
  0.142.*|0.143.*) _need_profile=1 ;;
esac
if [ -n "\$_active_profile" ] && [ "\$_active_profile" != "openai" ] && [ "\$_need_profile" = "1" ]; then
...
# Write {profile}.config.toml for each profile (v0.142+ new format)
for key, p in filtered.items():
    name = p.get('name', key)
    base_url = p.get('base_url', '')
    model = p.get('model', 'gpt-4o')
